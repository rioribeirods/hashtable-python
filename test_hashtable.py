...
def test_should_create_empty_value_slots():
    ...
    assert hash_table._buckets == [[], [], []]
    assert len(hash_table._buckets) == 3
    assert all(len(bucket) == 0 for bucket in hash_table._buckets)
...
def test_should_report_length_of_empty_hash_table():
    assert HashTable(capacity=100).size == 0
...
def test_should_resize_when_load_factor_high():
...
def test_should_clear_hash_table(hash_table):
    assert hash_table.size == 3
    hash_table.clear()
    assert hash_table.size == 0
    assert hash_table.capacity == 100  # Capacidade mantém

def test_should_pop_existing_key(hash_table):
    value = hash_table.pop("hola")
    assert value == "hello"
    assert "hola" not in hash_table
    assert hash_table.size == 2

def test_should_pop_with_default():
    ht = HashTable(10)
    value = ht.pop("missing", "default")
    assert value == "default"

def test_should_popitem():
    ht = HashTable(10)
    ht["a"] = 1
    ht["b"] = 2
...
def test_should_update_from_dict():
    ht = HashTable(10)
    ht.update({"a": 1, "b": 2})
    assert ht["a"] == 1
    assert ht["b"] == 2
...
def test_should_setdefault():
    ht = HashTable(10)
    value = ht.setdefault("key", "default")
    assert value == "default"
    assert ht["key"] == "default"
    
    value = ht.setdefault("key", "new_default")
    assert value == "default"  # Não muda
